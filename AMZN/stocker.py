...
#         return model, future
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        if not nshares:
            ...
            # 日期预测范围
            return "Prediction Range",start_date,end_date,"Predicted price",max(future['ds']),future.loc[future.index[-1], 'yhat'],"Actual price",max(test['ds']),test.loc[test.index[-1], 'y'],"Average Absolute Error on Training Data",train_mean_error,"Average Absolute Error on Testing  Data",test_mean_error,"the model predicted an increase, the price increased",increase_accuracy,"the model predicted a  decrease, the price decreased",decrease_accuracy,"actual value was within the",int(100 * model.interval_width),"confidence interval",in_range_accuracy
...
#             print('\nThanks for playing the stock market!\n')
            ...
            return "Date",self.symbol, start_date, end_date, nshares,"the model predicted an increase, the price increased",increase_accuracy,"the model predicted a  decrease, the price decreased",decrease_accuracy,"The total profit using the Prophet model",np.sum(prediction_profit),"The Buy and Hold strategy profit",float(test.loc[test.index[-1], 'hold_profit'])
...
#         print(future_decrease[['Date', 'estimate', 'change', 'upper', 'lower']])
        return future_increase[['Date', 'estimate', 'change', 'upper', 'lower']],future_decrease[['Date', 'estimate', 'change', 'upper', 'lower']]
